"""
KIDA — main UI  v2.2
Larger controls · Compact camera · Face/age/gender detection mode
"""
...
# Shared robot state (read by Flask /status)
_robot_state: dict = {
...
# Runs DeepFace on a copy of the latest frame every ~1.5 s so it never
# blocks the 25-fps render loop.  Results are read back safely via a lock.

_face_results: list = []        # list of {"gender":str, "age":int, "region":{x,y,w,h}}
_face_lock     = threading.Lock()
_face_frame_q: queue.Queue = queue.Queue(maxsize=1)   # holds latest PIL image
_face_enabled  = False          # controlled by mode switch


def _face_worker() -> None:
    """Background thread: pop frames from queue, run DeepFace, store results."""
    ...
    while True:
        if not _face_enabled:
            ...
            continue
        try:
            pil_img = _face_frame_q.get(timeout=0.5)
        except queue.Empty:
            continue
        try:
            import numpy as np
            ...
            for r in results:
                ...
                gender = max(gender_stats, key=gender_stats.get) if gender_stats else "?"
                parsed.append({
                    "gender": gender,
...
def _draw_face_overlays(surf, faces: list, cam_x: int, cam_y: int,
                        cam_w: int, cam_h: int,
                        native_w: int, native_h: int,
                        font_sm, font_xs):
    """Draw bounding boxes + gender/age labels scaled from native camera coords."""
    sx = cam_w / native_w
    sy = cam_h / native_h
    for face in faces:
        reg = face.get("region", {})
        ...
        pygame.draw.rect(surf, box_col,
                         pygame.Rect(x, y, fw, fh), 2, border_radius=4)
...
def main() -> None:
    ...
    # Native camera resolution (must match picamera2 config below)
    CAM_NATIVE_W, CAM_NATIVE_H = 320, 240

    # ── Fonts ─────────────────────────────────────────────────
    fmono_xl = pygame.font.SysFont("Courier New", 28, bold=True)
    ...
    logger.info("Flask + face threads started")
    ...
    # Face-scan button (below capture, right panel)
    face_btn_y = cap_y + cap_h + 16
    btn_face_snap = pygame.Rect(rp_x, face_btn_y, rp_w, 40)
    ...
    def save_face_snapshot() -> None:
        """Save current frame with face overlays drawn on top using PIL."""
        try:
            pil_base = cam_pil.copy() if cam_pil else None
            if pil_base is None:
                return
            ...
            for face in faces:
                ...
                draw.rectangle([x, y, x + w_, y + h_], outline=col, width=2)
                draw.text((x + 2, y - 14), f"{gender[0]} {age}y", fill=col)
            fname = os.path.join(face_dir, f"face_{int(time.time())}.jpg")
            pil_base.save(fname, "JPEG")
            logger.info("Face snapshot saved: %s", fname)
        except Exception as e:
            logger.error("Face snapshot failed: %s", e)
    ...
    def _process_command(cmd: str) -> None:
        ...
        if cmd in ("up", "forward"):
            ...
            mode = Mode.USER
        elif cmd == "_mode_autonomous":
            mode = Mode.AUTONOMOUS
        elif cmd == "_mode_line":
            mode = Mode.LINE
        elif cmd == "_mode_face":
            mode = Mode.FACE if hasattr(Mode, "FACE") else mode
        _update_state(direction=direction, speed=speed, mode=mode.name,
                      video_rec=video_rec, music_playing=music_playing)

    # ── Main loop ─────────────────────────────────────────────
    cam_pil   = None   # latest PIL image from camera (for face worker + snapshot)
    running   = True
    ...
    while running:
        ...
        # Camera capture every 3 frames
        cam_tick += 1
        if cam_tick >= 3:
            cam_tick = 0
            cam_surf, cam_pil = _cam_to_surface(cam, CAM_W, CAM_H)

        # Push frame to face worker every ~45 frames (~1.8 s at 25fps)
        if mode == Mode.FACE:
            _face_enabled = True
            face_tick += 1
            if face_tick >= 45 and cam_pil is not None:
                face_tick = 0
                try:
                    _face_frame_q.put_nowait(cam_pil.copy())
                except queue.Full:
                    pass
        else:
            _face_enabled = False
        ...
        info_items = [
            ...
            ("LED" if mode != Mode.FACE else "FACES", face_label, PURPLE if mode == Mode.FACE else SEC),
        ]
        ix = CAM_X
        ...
        # Face snap button (always visible, only useful in FACE mode)
        _btn(screen, fbody, "💾 SAVE FACES",
             btn_face_snap, mouse,
             hover_col=PURPLE,
             active=(mode == Mode.FACE))
        ...
        vers = fmono_xs.render("KIDA v2.2 · RASPBERRY PI", True, DIM)
        ...
        if mode == Mode.AUTONOMOUS:
            ...
            # Robot is stationary while scanning
            motors.stop()
            # Purple LED pulse to indicate face-scan active
            pulse = int((math.sin(frame * 0.2) * 0.5 + 0.5) * 128)
            set_led((pulse, 0, pulse))
