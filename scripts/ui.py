...
def _cam_to_surface(cam: Picamera2, w: int, h: int) -> pygame.Surface:
    try:
        ...
        if frame.shape[2] == 4:
            frame = frame[:, :, :3]
        pil = Image.fromarray(frame, "RGB").resize((w, h), Image.BILINEAR)
...
def _waveform(surf, frame: int, r: pygame.Rect, bars: int = 60,
    ...
    """
    Draw a waveform bar chart. Uses real AudioAnalyzer amplitudes when
    available; falls back to a sine animation or a flat idle state.
    """
...
def main() -> None:
    ...
    ctrl_scheme   = 1          # 1 = WASD, 2 = QA/WS tank
    music_playing = False
    video_rec     = False
    frame         = 0
    direction     = "STOPPED"
    ...
    def toggle_video() -> None:
        nonlocal video_rec
        if not video_rec:
            fname = os.path.join(videos_dir, f"video_{int(time.time())}.h264")
            try:
    ...
    def _process_command(cmd: str) -> None:
        """Apply a remote command from the Flask queue."""
